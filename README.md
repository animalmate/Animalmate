...
> 사이트 지원서 접수부터 서류 채점·면접 배정·면접 콘솔·최종 결정·비로그인 결과 조회·데이터 폐기까지 전 과정 동작
...
> **2026-08-03**: **동아리 일정 캘린더**(`/calendar`) 신설 — 총회·MT 를 문서가 아니라 표에 두고
> 챗봇이 tool 로 읽는다(고쳐야 할 문서를 없앤다). 챗봇이 **답하지 못한 질문**은 콘솔 카드와
> 주 1회 회장단 메일로 모아 낸다([`docs/07-DECISIONS.md`](docs/07-DECISIONS.md) 84~87).
> **2026-08-04**: **단톡 공지 지원** — 예약 큐의 `카카오톡 공지 예약` 이 공지문과 예약 시각
> (발행 +1분)을 조립해 준다. 보내는 것은 계속 사람이다(카카오는 단톡방 자동 전송 API 가 없다).
> 글 주소는 발행 전 알 수 없어 **모바일 게시판 주소**를 넣는다([`docs/07-DECISIONS.md`](docs/07-DECISIONS.md) 88).
> 메뉴 `일정` 을 **`캘린더`** 로 바꾸고, 문서 화면에 "일정은 캘린더에만" 안내를 붙였다(중복 방지).
> 남은 것 = 운영 작업(**팀별 가이드북** — 제작 후 챗봇이 링크로 안내한다),
...
- **RAG 챗봇**(로그인 전용, 전원): 회장단이 올린 안내 문서(마크다운)를 임베딩·검색해 답한다.
  **역할별 공개범위(visibility) 필터를 검색 SQL에서 강제**(부원에게 운영진 문서 미노출), 근거가
  ...
  사용자 데이터 분리). 봉사 회차·동아리 일정 같은 상태 질의는 문서 검색이 아니라 **DB tool**
  (`events` / `schedules`)로 답해 문서가 낡아도 답이 낡지 않는다. 인당 일일 + 전역
