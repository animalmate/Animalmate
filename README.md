...
## 발행 크론 (pg_cron → /api/cron/publish)
Vercel Cron 금지(규칙 #7). Supabase pg_cron + pg_net 으로 매분 발행 워커를 호출한다.
앱 배포(Vercel) 후 Supabase SQL 에디터에서 아래를 실행(플레이스홀더 교체):
```sql
-- 확장은 Dashboard > Database > Extensions 에서 pg_cron, pg_net 활성화.
...
- 워커는 매 호출 처리 요약(처리/성공/대기/실패/dryRun)을 JSON 으로 반환하고 audit_logs(`cron.publish`)에 남긴다 — 크론 관제 로그.
...
3. (선택) 매직링크·가입 안내 이메일 템플릿 한국어화.

### 준비되면
위가 끝나 `RESEND_API_KEY` 등 값이 채워지면 신호를 주세요 — 매직링크 로그인 + **학기별 가입코드**
가입 플로우(유효한 가입코드 없이는 가입 불가) 구현으로 이어갑니다.
