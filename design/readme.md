...
## VISUAL FOUNDATIONS
- **색**: 로고 추출 3원색 — 블루 `#5588D2`(primary, 신뢰/조작), 코랄 `#EE5A60`(위험/강조), 앰버 `#F0A72A`(대기/주의). 배경은 `#F6F5F2`, 카드는 흰색. 중립색은 아주 옅은 웜 그레이(ink).
  - ⚠ **2026-08-06 개정**: 원래는 채도를 얹은 크림(`#FAF6EE`)이 "따뜻함"을 담당했지만, cream·ink 전 계열이 색상각 38~42°에 몰려 **화면 전체가 누렇게 보인다**는 사용자 지적으로 채도를 1/3 로 내렸다(명도는 그대로 — 대비비 유지). 따뜻함은 이제 배경이 아니라 **로고 3원색과 해요체 문구**가 담당한다. 토큰 이름(`cream`·`ink`)은 그대로 두었다 — 이름을 바꾸면 전 화면을 건드려야 하는데 얻는 것이 없다.
