...
## VISUAL FOUNDATIONS
- **색**: 로고 추출 3원색 — 블루 `#5588D2`(primary, 신뢰/조작), 코랄 `#EE5A60`(위험/강조), 앰버 `#F0A72A`(대기/주의). 배경은 `#F8F5F0`, 카드는 흰색. 중립색은 아주 옅은 웜 그레이(ink).
  - ⚠ **2026-08-06 개정**: 원래는 채도를 얹은 크림(`#FAF6EE`)이 "따뜻함"을 담당했지만, cream·ink 전 계열이 색상각 38~42°에 몰려 **화면 전체가 누렇게 보인다**는 사용자 지적으로 **채도를 0.65 배**로 내렸다(명도·색상각은 그대로 — 대비비 유지). 온기는 남기되 누런 기만 뺀 지점이다. 따뜻함의 주역은 배경에서 **로고 3원색과 해요체 문구**로 옮겨 갔다. 토큰 이름(`cream`·`ink`)은 그대로 두었다 — 이름을 바꾸면 전 화면을 건드려야 하는데 얻는 것이 없다.
...
- **배경**: 단색(`cream-50`). 그라데이션·패턴 없음(AI-slop 회피). 사진/일러스트는 실제 활동 사진 자리에 placeholder.
