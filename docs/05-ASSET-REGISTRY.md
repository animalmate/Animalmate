...
| `team-guidebooks` | **비공개** | 팀 가이드북 PDF + 전체 부원 가이드북(`club/` 아래). 회원 전용 자료 | 생성 완료(2026-08-28 확인) |
...
**Public bucket 체크 해제**. 버킷 쪽 파일 크기 상한은 **비워 둔다**(현재 `file_size_limit=null`) —
그러면 프로젝트 전역값이 적용되고, 무료 플랜의 전역값이 곧 **50MB** 다. 앱도 같은 50MB 로 막는다
(`MAX_GUIDEBOOK_BYTES`). 여기에 더 작은 값을 넣으면 앱은 통과시키는데 업로드가 413 으로 잘려
"파일을 올리지 못했습니다"만 뜬다 — 화면이 이유를 설명할 수 없는 자리다.

> 실측(2026-09-03): 49MB PUT → 200, 55MB PUT → **413 `EntityTooLarge`**. 유료로 올려도 Gemini 의
> PDF 상한(50MB)이 그대로라 50MB 가 끝이다.
