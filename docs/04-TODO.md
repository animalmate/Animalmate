...
- [ ] **33기 실운영에서 나온 다음 할 일 (2026-08-20)** — 첫 실제 기수(지원자 203명)를 돌리며 나온 것들.
  ...
  - [ ] **운영 DB 에서 규모 리포트 한 번 돌리기** — `node scripts/recruit-scale-report.mjs`
        (읽기 전용·개인정보 미출력). 33기 실제 자기소개 길이·채점 진척·면접 슬롯 현황을 확인한다.
        위 실측은 **가짜 600자** 기준이라, 실제 글이 더 길면 첫 로드도 그만큼 무거워진다.
