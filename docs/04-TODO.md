...
- [x] **34기 준비 묶음(2026-08-26)** — 33기를 돌려 보고 34기 접수 전에 필요해진 것들.
  ...
  - [ ] **영문 이름을 어디서 볼지 정한다** — 합격 시 외부 단체(로타랙트) 가입에 쓰는 값인데
        **저장만 되고 어느 화면에도 안 나온다**(지금은 DB 를 직접 열어야 한다). OT 참가 여부는
        2026-08-26 에 서류 심사 화면에 붙였다. 영문 이름은 붙일 자리가 **합격자 명단을 뽑는 화면**인데
        그게 아직 없다. 34기 최종 발표 때 실제로 필요해지면 그때 만든다.
