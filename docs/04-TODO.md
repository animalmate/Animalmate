...
- [x] **면접 전날 정리 (2026-08-21)** — 근거는 `07-DECISIONS.md` 123~127.
  ...
  - [ ] **`recruit_mapping_presets` 테이블이 고아가 됐다** — CSV 업로드(결정 123)를 지우면서
        이 테이블을 읽고 쓰는 코드가 전부 사라졌다. 스키마에만 남아 있다. 33기 열 매핑이라
        앞으로 쓸 데가 없지만, **테이블 드롭은 되돌릴 수 없어** 급하지 않은 지금 하지 않는다.
        지울 거면 마이그레이션으로(`db:generate` → 운영·테스트 둘 다 `db:migrate`).
