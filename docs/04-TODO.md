...
- [x] **모집 화면 UX 정비(2026-07-28)**: 단계 내비 역할별 자물쇠 표시(32) / 공고 설정 필드 단위 권한으로
      홍보팀 개방(31 — 2026-07-31 결정 66 으로 번복 → **2026-08-25 결정 140·141 로 다시 개방**,
      아래 항목 참고) / 예약 취소·팀 삭제 확인 추가(33) / 홈 바로가기에 신입모집 추가.
- [x] **홍보팀 공고 편집 권한(2026-08-25, 결정 140·141)**: `teams.can_edit_notice`(마이그레이션 0032,
      기본 false)를 회장단이 **회원 → 팀 관리**에서 팀 단위로 켠다. 팀 **이름**으로 판단하지 않는다 —
      옛 `isPRTeamOrPrivileged` 는 UUID 를 이름처럼 비교해 항상 false 였고(결정 66) 팀명은 매 학기 바뀐다.
      판단은 `canEditRecruitNotice(actor)` 한 곳, `loadActor` 가 `can_edit_notice AND is_active` 로 채운다.
      **범위**: 0번 화면 전부(공고 본문·포스터·지원서 문항·기수 생성·마감 스위치·합격자 안내문·
      면접 장소·대기실 업무) + 지원자 공개 스위치. **회장단에 남은 것**: 기수 삭제 + 합격을 정하는 행위
      (서류/최종 확정·면접 배정·지원자 팀 변경·폐기·export).
      경계는 "대외에 나가느냐"가 아니라 **"합격을 정하느냐"**다. 운영 DB 적용 완료, 홍보팀 플래그 ON.
...
- [x] **평가셋 33문항 + 오답률 측정(2026-08-28, 결정 152·153)** — `npm run eval` → `eval/results/latest.md`.
      옛 파일은 `"실제 문항으로 교체하세요"` 라고 적힌 예시 5개였고 회비를 2만원이라 적어 둔 것까지
      있었다(실제 5만원). 첫 실행 30/33(91%)에서 **회비·활동 기간이 핸드오프되는 실제 결함**을 잡았고
      (청킹이 원인 — 결정 153), 고친 뒤 32~33/33(97%)이다.
      ⚠ **주간 기록은 아직 자동화하지 않았다.** 지금은 사람이 `npm run eval` 을 돌리고 리포트를 본다.
      실행마다 한 건쯤 흔들리므로(temperature 0.2) 한 번의 숫자를 결론으로 읽지 말 것.
- [x] **audit log 조회 화면(회장단) — `/admin/audit`**(2026-08-28, 결정 154). 기본은 사람이 한 일만
      (전체 12,988건 중 87%가 `cron.publish`). 분류·행위자·기간·주의표시 필터 + 이어보기.
      브라우저 워크스루 16항목 통과(필터·이어보기 중복 없음·모바일 카드·부원 403·콘솔 오류 0).
      **CSV export 는 넣지 않았다** — 감사 기록에는 이전값·새값이 그대로 들어 있어 export 는 새로운
      PII 유출 경로가 된다. 필요해지면 그때 범위를 정해서 만든다.
- [x] ~~지표 대시보드~~ — **만들지 않기로 했다(2026-08-28).** 무엇을 재야 하는지 정해진 적이 없고,
      지금 실제로 보는 숫자는 이미 제자리에 있다(발행 성패=예약 큐 배지, 답 못한 질문=`/admin/chatbot`,
      챗봇 품질=평가셋 리포트, 나머지=기록 화면). 대시보드를 세우면 **같은 사실의 출처가 하나 더 생겨**
      어긋날 자리만 늘어난다. 필요해지면 "무엇을 보고 무엇을 결정할지"부터 정한다.
...
- [x] **34기 준비 묶음(2026-08-26)** — 33기를 돌려 보고 34기 접수 전에 필요해진 것들.
      근거는 `07-DECISIONS.md` 143~148.
  - [x] **채점 되돌리기** — 면접 콘솔·서류 심사 양쪽에 **내 점수 지우기**(결정 143·147).
        `DELETE /api/recruit/scores` 는 **원래 있었고 아무도 부르지 않았다** — 서버는 멀쩡한데 버튼이
        없었다. 낮은 점수로 덮어쓰는 것은 되돌리기가 아니다(0점은 0점을 준 것이고, 서류에서는
        집계의 '채점 인원'에도 계속 잡혀 표본 판정이 거짓이 된다).
        서버가 `scorer_user_id` 로 좁혀 지우므로 남의 점수는 못 건드린다.
  - [x] **중복 지원 = 마지막 것만 남긴다**(결정 144, 사용자 지시). 409 거절을 걷어내고 **제자리에서
        갈아 끼운다** — 새 행을 만들면 면접 배정·점수·메모가 끊긴다. `assigned_team` 은 운영진이
        손대지 않았을 때만 새 1지망을 따라간다. 기수 행 `FOR UPDATE` 로 두 번 누르기 경합까지 막는다.
        ⚠ 채점이 끝난 뒤 재제출하면 채점자가 읽은 글이 바뀐다 — 실무 방어는 **심사 전 마감 스위치**이고,
        감사 로그가 `[high]` 로 알려 준다.
  - [x] **지원서 문항 사용 스위치**(결정 146, 사용자 요청). 끄는 방법이 제목 지우기뿐이라 껐다 켜면
        문구를 잃었다. `FieldConfig.enabled` 를 더하고 판정을 `isFieldOn()` 한 곳으로 모았다.
        **마이그레이션 없음** — `enabled` 없는 옛 jsonb 는 읽는 순간 빈 제목으로 해석된다.
        추가모집처럼 OT 이후에 들어오는 기수는 OT 문항만 끄면 된다.
        같이 고친 것: 편집기의 `disabled={locked && false}` 죽은 코드(이름·전화번호 제목이 비워졌다).
  - [x] **회장단 전용 문서를 `/documents` UI 에 연다**(결정 145) — 결정 19 를 뒤집었다.
        데이터 모델은 처음부터 3단계였는데 UI 만 2단계라, 이미 board 인 문서를 편집하러 열면
        드롭다운이 빈칸이 되고 되돌릴 수 없었다.
  - [x] **결과 안내 메일**(결정 148, 사용자 요청) — 마이그레이션 0033 `recruit_result_mails`.
        공개 스위치에 **자동으로 걸지 않는다**(스위치는 되돌릴 수 있고 메일은 아니다).
        사람이 버튼을 누르고, 누르기 전에 몇 명에게 나가는지 본다. 단계 셋(서류·면접 일정·최종).
        **메일에 당락을 쓰지 않는다** — 단위 테스트가 당락 단어를 금지했고 실제로 하나 잡았다
        ("최종 **합격** 결과 안내" → "최종 결과 안내").
        중복 방어는 `(applicant_id, stage)` UNIQUE. 하루 400통(Gmail 500 을 인증 메일과 나눠 쓴다),
        넘친 것은 다음 날로. `pg_cron result-mails`(jobid 5, 5분) **등록·확인 완료**.
  - [x] **챗봇 지식베이스에 사이트 안내 3종 적재**(부원·운영진·회장단). 그전까지 `docs/rag/*.md` 는
        리포에만 있고 `/documents` 에 올라간 적이 없어 **챗봇이 홈페이지 사용법을 몰랐다.**
        공개 범위를 손으로 고르다 틀리는 사고를 막으려고 표를 코드에 박은
        `scripts/sync-rag-docs.ts` 를 만들었다(미리보기 기본, `--apply` 로 저장).
  - [x] **34기 접수 전 실사용 점검(2026-08-28 완료, 결정 155)** — **운영이 아니라 테스트 DB 에서** 돌렸다.
        운영의 `테스트` 기수 50명은 주소가 전부 `@example.invalid` 라, 대기열에 담겨 발송까지 가지만
        **반드시 실패한다** — 성공 경로가 아니라 실패 경로만 밟으면서 운영 `recruit_result_mails` 에
        `failed` 행과 Gmail 반송을 남긴다. 게다가 그 기수는 공개 스위치가 꺼져 있어 리허설을 하려면
        **운영 기수의 스위치를 켰다 꺼야** 한다(지금 33기 추가모집이 접수 중이다).
        · **로직** — `test/recruit-result-mails.service.test.ts` 9케이스(미리보기 정확도 · 두 번 눌러도
          한 통 · 워커 발송과 `sent` 확정 · 단계별 대상 · 스위치 차단 · 깨진 주소는 시도 없이 실패 ·
          재시도 3회 확정 · 25통 상한 · 기수 경계). **역검증까지 했다** — 슬롯 조건과 이메일 가드를
          되돌리면 정확히 그 두 케이스가 깨지는 것을 확인.
        · **배선** — 운영에서 읽기 전용으로 확인: pg_cron `result-mails`(jobid 5)가 2시간에 24회
          정확히 실행, `net._http_response` 가 **200**. 크론이 성공해도 라우트가 401 이면 한 통도
          안 나가므로 HTTP 응답까지 본다.
        · ⚠ 33기 지원자 중 **이메일 칸에 주소가 아니라 문장이 든 행이 1건** 있다(서버 검증이 생기기
          전에 접수). 형식 검사에 걸려 대상에서 빠지고 화면의 "보낼 수 없는 사람" 수에 잡힌다 —
          **그 사람에게는 운영진이 따로 연락해야 한다.** 오타 도메인(`naver.con`) 2건도 형식은
          유효해서 큐에 담기고 반송된다.
  - [x] **영문 이름을 6번 최종 결정 화면에 붙였다**(2026-08-26, 사용자 지정 "recruit/final 에 뜨게 하자").
        합격 시 외부 단체(로타랙트) 가입에 쓰는 값인데 그전까지는 **저장만 되고 어느 화면에도
        안 나와** DB 를 직접 열어야 했다. 자리는 두 곳이다:
        · **결과 미리보기 표** — 합격 예정인 사람의 이름 아래에 함께 보인다(확정 전에 눈으로 훑는 용도).
        · **최종 합격자 명단 카드** — `final_pass` 가 생겼을 때만 뜬다. 미리보기 표는 `interview_done`
          만 계산해서 **확정 버튼을 누르는 순간 텅 빈다** — 정작 명단이 필요한 때가 그 다음이라
          확정 뒤에도 남는 자리가 따로 있어야 했다. 팀 → 이름 순으로 세우고, 안 적은 사람은
          `영문 이름 미입력 N명` 으로 먼저 알린다(명단을 넘기기 전에 알아야 하는 사실).
        불합격자에게는 보이지 않는다 — 개인정보 처리방침이 **최종 합격자에게만 사용**이라고 적었다.

- [x] **번개 게시판(2026-09-04, 사용자 요청 — 07-DECISIONS 162)** — 카톡 공지 + 개최자 개인톡으로
      굴러가던 즉흥 소모임을 게시판으로 옮겼다. 옮긴 이유는 **순서가 어디에도 안 남기 때문**이다
      (개인톡은 누가 먼저 말했는지 개최자 머릿속에만 있어, 정원이 찼을 때 근거가 없다).
      마이그레이션 `0036` (`flash_meetups`/`flash_hosts`/`flash_signups`/`flash_messages`, RLS 손으로 추가).
  - [x] **개최 — 부원도 낸다.** `flash.create` 는 부원 이상 전원이고, 부원 것은 `pending`(운영진 승인
        대기), 운영진 이상은 곧바로 `open` 이다. 권한이 아니라 **첫 상태**로 가르는 이유는 부원을
        authorize 에서 막으면 "개최 신청"이라는 기능이 존재할 수 없기 때문이다.
        승인·거절은 `flash.approve` = **운영진 전원**(회장단 전용 아님). **거절 사유 필수.**
  - [x] **공동 개최** — 개최자는 `flash_hosts` 표로 여럿이다. `flash.manage` 는 이 리포에서 유일하게
        `Ownership(personal|team)` 을 안 쓰는 액션이고 개최자 여부로 판단한다(회장단 override).
  - [x] **선착순 + 대기 줄** — 순번(`seq`)은 번개 행을 `FOR UPDATE` 로 잠근 트랜잭션 안에서만 딴다.
        정원이 차도 신청을 받고 대기 번호를 준다. 자리 배정은 매번 seq 순서로 **처음부터 재계산**
        (`assignSeats` 순수 함수 → `resyncSeats`)이라 취소 승격·정원 축소가 별도 코드 없이 맞는다.
        취소 후 재신청은 **새 번호**(대기 줄 맨 뒤).
  - [x] **메시지가 곧 신청** — 빈 신청 버튼이 없다. 첫 쪽지가 신청 행을 만들고 그대로 1:1 대화가 된다.
        **쪽지는 당사자와 개최자만 본다 — 회장단도 예외가 아니다**(관리 권한은 글에 대한 것).
  - [x] **알림은 사이트 안에서만**(사용자 결정) — 메일 없음. 홈 카드·게시판 목록의 안 읽음 배지.
        읽음 시각은 **개최자마다·신청자마다** 따로 둔다(공동 개최자 한 명이 열어도 나머지 배지가 안 꺼진다).
  - [x] **챗봇 tool `list_flash_meetups`** — `from` 에 과거 날짜를 주면 개최 내역. **이름은 안 싣는다**(인원 수만).
  - [x] 단위 25 + 통합 25 + 권한 9케이스. RLS 증명 91 → **103** 통과.
  - [x] **신청 시작 시각(2026-09-04, 결정 163)** — "9월 30일 오후 3시부터" 를 정해 두면 모두가 같은
        출발선에 선다. `신청 예정` 은 상태가 아니라 파생값(`signupWindow`)이라 시각이 지나면 크론 없이
        저절로 열린다. 창을 여는 판단은 서버 한 곳(`signUpToFlash`)이고, 화면은 `serverNow` 와의 차이
        위에서 카운트다운만 한다. 시각은 `timestamptz` 한 칸 + KST 벽시계 변환 한 곳(`kstLocalToInstant`).
  - [x] **개최자 전체 안내(같은 결정)** — 확정·대기 전원의 1:1 방에 같은 한 줄을 넣는다.
        공지판을 따로 두지 않았고, 회장단 override 는 여기서만 막았다.
  - [x] **UI/UX 정리(같은 결정)** — 개최자 화면 명단 중복 제거 · 신청 칸을 명단 위로 · 대화창 높이
        제한과 자동 스크롤 · 취소된 신청 접기 · `모집 중`/`신청 예정` 분리 · 목록 날짜에 오늘·내일.
  - [x] **명단에 미리 넣기(2026-09-05, 결정 166)** — 개최자가 "이 자리는 운영진 몫" 처럼 자리를 먼저
        잡아 둔다. 숫자를 깎지 않고 **신청 행**으로 넣어서(`flash_signups.placed_by`) 대기 승격·본인
        취소·내보내기가 기존 경로 그대로 굴러간다. **자리를 빼앗지 않는다**(순번 맨 뒤, 정원이 찼으면
        대기). 신청 시작 시각 전에도 넣을 수 있고(그게 목적), 승인 전 번개에는 못 넣는다.
        전부 되거나 전부 안 된다. 명단에 `개최자가 넣음` 딱지가 그대로 보인다.
        마이그레이션 `0038`. 단위 4 + 통합 12 추가.
  - [ ] **슬롯(주최자가 만든 선택지 중 하나를 고르는 신청) — 만들지 않기로 함(2026-09-05 사용자 결정).**
        신청은 지금처럼 받는다. 고를 것은 세부 내용에 적고 신청 메시지로 말한다.
  - [ ] **실사용.** 첫 번개를 여기서 열고 카톡 공지에 `신청은 홈페이지 번개 게시판에서` 한 줄을 붙이는 것.
        코드가 아니라 사람 쪽 일이다(10-STATUS §5-2).
