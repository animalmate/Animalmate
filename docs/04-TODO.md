...
- [ ] **33기 실운영에서 나온 다음 할 일 (2026-08-20)** — 첫 실제 기수(지원자 203명)를 돌리며 나온 것들.
  ...
  - [x] **1순위 · 결과 발표 전 필수: `recruitLookup` 상한을 인원 규모에 맞춘다.** — 2026-08-20 구현
        ...
        **2026-08-20 배포 완료.**
  - [ ] **2순위 · 203명 규모 점검: 서류 집계·확정 → 면접 슬롯.**
        이 시스템이 실측해 본 최대 규모는 `테스트` 기수 **50명**이었다. 203명은 4배다.
        · 서류 심사 화면은 자기소개 전문을 포함한 **전체 조회**를 쓴다(주석 기준 50명에 60.9KB).
        · 집계·배정 화면은 `listApplicantsByCohortSlim` 이라 상대적으로 가볍다.
        · 면접 슬롯 개수가 203명 기준으로 충분한지 미리 센다.
  ...
  - [ ] **3순위 · 업로드 전화번호 형식 가드** — 정규화 **후** 값으로 판정한다(`isValidPhone` 이 이미
        `src/lib/phone.ts` 에 있다). 지금은 `전화번호: "없음"` 같은 값이 빈 문자열로 저장되고
        **빠진 행 목록에도 안 뜬다** — 그 사람은 결과 조회가 영구히 불가능하다(`phone` 컬럼이
        `notNull` 이지만 `''` 는 통과한다). 엑셀이 다시 저장한 CSV 의 지수표기(`1.01E+10`)도 같은
        구멍으로 들어온다. 33기 파일엔 둘 다 0건이었지만, **인계(2027-1학기) 뒤에는 사람이 대신
        봐 줄 수 없다.** 등록을 막을지 경고만 할지는 붙일 때 정한다.
  - [ ] **3순위 · 미리보기가 중복쌍의 제출 시각을 함께 보여 준다** — 결정 117. 지금은 "중복 N명"만
        나와서, 그게 재제출인지(=고친 쪽이 버려지는 중인지) 알 방법이 화면에 없다.
  - [ ] **3순위 · 주소 경고 배지가 실제로 잡게 한다** — 지금 배지는 **미리보기 샘플 5행에만** 그려지고
        판별식이 `/[시구동번지]/` 라 성기다. 33기에서는 진짜 상세 주소 1건이 샘플 밖이라 안 떴고,
        `둔촌동역` 같은 멀쩡한 역명이 오히려 걸린다. 전 행을 세어 "주소로 보이는 값 N건"으로 알린다.
  ...
  - [ ] **3순위 · `fix/recruit-csv-upload` 브랜치 정리** — main 에 fast-forward 병합됐다(48f7dd1).
        원격 브랜치가 같은 커밋을 가리키고 있으니 지워도 된다.
