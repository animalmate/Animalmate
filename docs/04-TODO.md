...
> **실카페 발행 전환 완료(2026-07-24)**: 봇 계정 카페스탭 임명 + Vercel `NAVER_PUBLISH_DRY_RUN=false`.
> 이제 예약이 실제 카페에 게시된다 — **카페는 수정·삭제 API 가 없어 한번 나간 글은 되돌릴 수 없다.**
> (로컬 `.env` 는 `true` 로 둔다. 개발 중 실수로 실게시되는 것을 막는 안전장치 — 바꾸지 말 것.)
> **남은 것: 챗봇(1D).**
>
...
- [~] **F9 신입 기수 모집(2026-07-25 착수, 스펙 대폭 확장)** — 상세 설계 = `docs/09-RECRUIT-DESIGN.md`.
  ...
  - [ ] **1b 서비스**(순수 로직 + 단위테스트): CSV 파서·매핑·중복감지 / 채점(면접 점수 최초 저장 시
        interview_done 자동 전환, 0개로 감소 시 doc_pass 자동 복귀, `nextStatusOnScoreChange`) /
        개인메모·공용메모지 / 집계(평균·최고/최저·표본부족) / 조회 매칭(이름+전화 정확 일치) / 폐기.
  - [ ] **2 API 라우트 + 업로드·매핑 UI**(회장단, 열↔필드 매핑·프리셋·미리보기·중복감지).
  - [ ] **3 서류 심사 화면**(운영진, 목록↔우패널·타인 점수 표시·정렬) + 공용 메모지 컴포넌트.
  - [ ] **4 서류 집계·확정 + 면접 배정**(회장단, 슬롯 격자·비대면 희망 표시).
  - [ ] **5 면접 당일 콘솔**(운영진, 자동 저장·좌우 분할·현재 시간대 강조).
  - [ ] **6 최종 결정 + 공개 스위치(2단계 확인) + 데이터 폐기**(회장단, "면접 기록 없음" 경고·noshow).
  - [ ] **7 `/recruit` 비로그인 조회**(noindex·독립 레이아웃·이름+전화 정확 일치·IP 분당 5회/실패 10회 차단,
        시도값 미저장) + nav/home 링크 + 07-DECISIONS 24·25 정식화.
  - [ ] **모집 종료 후 지원자 데이터 폐기**(회장단, 2단계 확인, 되돌릴 수 없음): 인적사항·자기소개서·메모·
