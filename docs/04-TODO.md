...
- [x] **34기 준비 묶음(2026-08-26)** — 33기를 돌려 보고 34기 접수 전에 필요해진 것들.
  ...
  - [ ] **34기 접수 전 실사용 점검** — 결과 안내 메일을 **테스트 기수(50명)로 먼저 한 바퀴** 돌린다.
        `TEST_DATABASE_URL` 이 없어 통합 테스트를 못 돌렸고(순수 규칙 단위 17개까지만 검증),
        새 테이블이 생긴 작업이라 실사용 전 확인이 필요하다. ⚠ 테스트 기수 이메일이 진짜 주소면
        실제로 메일이 나간다 — 가짜 주소인지 먼저 확인할 것.
