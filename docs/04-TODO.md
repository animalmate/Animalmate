...
- [x] **모집 화면 UX 정비(2026-07-28)**: 단계 내비 역할별 자물쇠 표시(32) / 공고 설정 필드 단위 권한으로
      홍보팀 개방(31) / 예약 취소·팀 삭제 확인 추가(33) / 홈 바로가기에 신입모집 추가.
- [x] **작업 B — 사이트 활용 가이드 3종(2026-07-28)**: 부원용/운영진용/회장단용(시기별 체크리스트 +
      문제 대응). 원문은 `src/guides/content.ts` 한 곳, `docs/08-USER-GUIDES.md` 는
      `node scripts/build-guides-doc.mjs` 로 생성(화면과 문서 불일치 방지). 노출 = 홈 "사용 가이드" 카드 +
      상단 메뉴 "가이드", 역할별(부원=1종 / 운영진=2종 / 회장단=3종, 서버에서 필터).
