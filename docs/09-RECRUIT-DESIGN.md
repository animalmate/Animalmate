...
## 5. CSV 업로드·매핑 (회장단)
- 흐름: 파일 선택(.csv) 또는 붙여넣기 → **바이트 검사·디코딩**(아래) → 헤더 파싱 →
  **열↔필드 매핑 UI**(각 지원자 필드에 CSV 헤더 셀렉트) → 프리셋 저장/불러오기 →
  **미리보기**(상위 5행 + 매핑 결과 + 빠진 행 + 중복 수) → 확정 업로드.
- 파싱은 순수 모듈(`src/recruit/csv.ts`): 따옴표·개행 포함 CSV 안전 파서 + 헤더 매핑 적용 + 중복 판정.
  자기소개서에 쉼표·줄바꿈이 많아 **정규식 split 금지**, 상태 기반 파서로 작성(단위 테스트 필수).
  구분자는 쉼표/탭 자동 판별(스프레드시트에서 Ctrl+C 하면 클립보드에 담기는 것은 **탭 구분**이다).
- **파일 읽기**(`src/recruit/csv-file.ts`, 순수): `File.text()` 를 그냥 쓰지 않는다 — 무엇을 받았든
  UTF-8 로 읽어 `.xlsx`(zip)가 `PK♥♦…` 로, 엑셀이 저장한 CP949 가 `�̸�` 로 들어왔다.
  순서는 **못 쓰는 형식(zip·OLE·NUL) → UTF-8 → CP949**. 못 쓰는 형식은 받는 방법을 알려 주며 막고,
  CP949 는 되살려 읽는다(막기만 하면 사람이 깨진 글자를 이름으로 저장한다). 상한 4MB. 결정 115.
- **값 정규화는 CSV 경로에만 적용한다**(사이트 자체 지원 폼은 시트를 거치지 않는다).
  - `phone`: 숫자만 남기는 데서 그치지 않고 **앞 0 과 국가번호 82 를 되돌린다**. 시트를 거치면
    `01012345678` 이 수로 해석돼 `1012345678` 로 오고, 그대로 저장하면 그 지원자는 결과 조회를
    못 한다(조회는 이름+전화 완전 일치). 애매한 값은 손대지 않는다. 결정 113.
  - `birth_date`: 해석이 **확실할 때만** `YYYY.MM.DD` 로 맞춘다(`YYYYMMDD`·`YYYYMM.DD`·`YYMMDD`·
    구분자 혼용까지). 13월·45일 같은 값은 원문을 그대로 둔다. 결정 114.
    ⚠ 등록 후에는 어느 화면에서도 못 고친다 — 그래서 업로드 시점에 맞춘다.
- **등록되지 못한 행을 세어 돌려준다**(`mapRowsToApplicants` → `invalidCount`/`invalidRows`).
  이름이나 전화가 비면 조용히 버려지는데, 숫자만 보여 주면 50명을 올리고 48명이 들어가도
  사람이 알아챌 방법이 없다. 미리보기에 10행까지 위치와 남은 값을 함께 띄운다. 결정 116.
- **중복은 이름+전화 완전 일치로 자동 제외한다**(덮어쓰기 선택지는 없다). 같은 파일 안 중복도 막는다.
  ⚠ **남는 쪽은 먼저 낸 것**이다. 재제출(고쳐서 다시 낸 지원서)이면 고친 쪽이 버려지므로,
  업로드 전에 사람이 CSV 에서 옛 줄을 지운다. 결정 117.
- `near_station`: 매핑 대상은 "가까운 역" 필드. 폼이 전체 주소를 받았다면 미리보기에서 주소로 보이는
  값에 **경고 배지**를 띄우고, 업로드 후 심사 화면에서 인라인으로 역명만 남긴다.
  (자동 축약은 신뢰 불가 → 사람이 정리.)
  ⚠ 지금 배지는 **샘플 5행에만** 그려지고 판별식이 `/[시구동번지]/` 라 성기다 — 33기에서는 진짜
  상세 주소 1건이 샘플 밖이라 안 뜨고, `둔촌동역` 같은 역명이 오히려 걸린다. 04-TODO 에 남겼다.
- 매핑 프리셋은 global(회장단 관리, 이름으로 재사용). value = `{applicantField: csvHeader}`.
- 실제 응답 파일 회귀 테스트: `src/recruit/csv-sample.test.ts` 가 구글 폼 18문항 응답을 고정한다.
