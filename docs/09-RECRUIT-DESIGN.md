...
5. **최종 검토 `/recruit/review`** (staff+, **읽기 전용**): 면접을 본 지원자를 **팀별로 묶어** 놓고
   서류 평균·면접 평균·**면접 평가 총평**(`recruit_scores.comment`, stage=interview)을 함께 본다.
   ...
   - 펼친 칸은 **지망 팀(1·2순위) + 면접 총평**이다(2026-08-23 개정). 서류 코멘트는 뺐다 —
     서류 이야기는 2번에서 끝났고, 목록이 배정팀(없으면 1지망)으로 묶여 있어 사람을 옮길지
     이야기하려면 1·2순위가 필요하다. 서류 **점수**는 표에 그대로 남는다. 개인 메모(`recruit_memos`)는
     **넣지 않는다** — 면접 콘솔에서 "나만 봅니다"라고 적어 두고 받은 값이다.
   - 이 화면에는 상태를 바꾸는 요청이 하나도 없다. 새 API·새 테이블도 없다(기존 staff+ 조회 3개 재사용).
