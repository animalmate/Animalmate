...
- 흐름: 파일 선택(.csv) 또는 붙여넣기 → **바이트 검사·디코딩**(아래) → 헤더 파싱 →
  ...
  **미리보기**(상위 5행 + 매핑 결과 + 빠진 행 + **중복쌍** + **주소로 보이는 값 건수**) → 확정 업로드.
...
- **중복은 이름+전화 완전 일치로 자동 제외한다**(덮어쓰기 선택지는 없다). 같은 파일 안 중복도 막는다.
  ...
  - **미리보기가 그 판단 재료를 준다**(결정 120): `빠지는 행 번호 · 이름 · 제출 시각 → 남는 행 번호 ·
    제출 시각`을 10건까지 적는다. 이미 등록된 지원자와 겹치면 "이미 등록된 지원자입니다"로 적는다.
    제출 시각은 구글 폼 **타임스탬프 열을 그때만 읽고 저장하지 않는다**(`findTimestampColumn`).
    열이 없어도 판단은 된다 — 구글 폼 CSV 는 시간순이라 행 번호가 곧 제출 순서다.
- `near_station`: 매핑 대상은 "가까운 역" 필드. 폼이 전체 주소를 받았다면 미리보기에서 주소로 보이는
  값에 **경고 배지**를 띄우고, 업로드 후 심사 화면에서 인라인으로 역명만 남긴다.
  (자동 축약은 신뢰 불가 → 사람이 정리.)
  - **판정은 `src/recruit/near-station.ts` 한 곳**에 있고 화면·서버가 같이 쓴다(결정 121).
    역 이름으로 끝나면 역명(`둔촌동역`·`고덕역 3번 출구`), 번지·도로명·건물번호가 보이거나 행정구역이
    두 단계 이상이면 주소. 애매하면 **역명 쪽으로** 판정한다 — 헛경보가 쌓이면 진짜 한 건이 묻힌다.
  - **세는 대상은 등록될 전 행**이다. 미리보기 샘플 5행에만 배지를 그리던 때는 33기의 진짜 상세 주소
    1건이 6번째 행이라 아무에게도 보이지 않았다. 배지는 샘플 표에 그대로 두고, 건수는 따로 알린다.
