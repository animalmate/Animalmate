...
1. **Vercel Hobby Cron 금지**: 하루 1회 제한 + 시간 단위 정밀도(지정 시각 미보장) + 재시도 없음.
   ...
   **함수 타임아웃은 300초다**(2026-07-31 사용자 확인: Fluid compute 활성 + Max Duration 300).
   여기 오래 적혀 있던 "10초"는 틀린 값이었다 — 그게 맞았다면 발행 워커의 **건별 30초 대기**
   (code 999 회피)가 2건째부터 반드시 타임아웃이라 이 문장과 워커 설계는 서로 모순이었다.
   여태 안 터진 이유는 **다건 사이클이 운영에서 한 번도 돌지 않았기** 때문이다
   (전부 1건, 카페 API 450ms, 사이클 0.6초, `publishing` 에 멈춘 예약 0건).
   - 지금 예산으로 한 사이클 최대치(5건 = 대기 4×30초 + API ≈ 125초)는 여유 있게 들어간다.
   - 이 값은 `src/app/api/cron/publish/route.ts` 에 `maxDuration` 으로 **명시**해 뒀다.
     기본값에 맡기면 조용히 바뀌었을 때 증상이 "2건째부터 10분씩 지연"이라 알아채기 어렵다.
   - 예산이 모자라더라도 **중복 게시는 아니다** — 점유(`publishing`)+임차 회수와
     `status='publishing'` 조건부 UPDATE 가 막는다. 증상은 지연이다.
