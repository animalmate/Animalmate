...
1. **Vercel Hobby Cron 금지**: 하루 1회 제한 + 시간 단위 정밀도(지정 시각 미보장) + 재시도 없음.
   정시 발행 불가 → 모든 스케줄은 pg_cron으로. 발행 워커는 1회 실행당 소량(≤5건)만 처리한다.
   ⚠ **함수 타임아웃 수치는 확인이 필요하다**(2026-07-31 QA). 여기 오래 적혀 있던 "10초"는
   Vercel 문서 기준으로 **더 이상 맞지 않는다** — fluid compute(현재 기본값)에서 Hobby 는
   기본·최대 모두 **300초**다. 옛 수치가 맞다면 발행 워커의 **건별 30초 대기**(code 999 회피)가
   2건째부터 반드시 타임아웃이라, 이 문장과 워커 설계는 애초에 서로 모순이었다.
   - **실제 영향은 아직 없다**: 운영에서 돈 사이클은 전부 1건짜리였고(카페 API 450ms, 사이클 0.6초),
     `publishing` 에 멈춘 예약도 0건이다. **다건 사이클은 운영에서 한 번도 실행된 적이 없다.**
   - **확인할 것**(Vercel 대시보드 → Settings → Functions): Fluid compute 활성 여부와
     Function Max Duration. 300초면 5건×30초=150초가 들어가므로 현행 그대로 두면 된다.
     60초 이하라면 한 사이클 처리 건수를 2건으로 낮춘다(1회 대기 ≈ 32초).
   - 최악이어도 **중복 게시는 아니다** — 점유(`publishing`)+임차 회수와 `status='publishing'`
     조건부 UPDATE 가 막는다. 증상은 2건째 이후가 임차 만료(10분) 뒤에야 나가는 **지연**이다.
