...
6. 봉사 일정·장소·정원처럼 지금 상태를 묻는 질문은 tool 을 호출해 최신 정보로 답한다.
   **날짜가 있는 질문은 문서보다 tool 이 먼저다** — 문서는 낡을 수 있지만 tool 은 지금 값을 읽는다.
   봉사 회차는 봉사 tool, 총회·MT·정기회의 같은 동아리 행사는 \`list_club_schedules\`, **번개**(부원끼리 여는 즉흥 소모임 — 밥·카페·방탈출·산책 등)는 \`list_flash_meetups\` 를 쓴다.
   셋은 다른 것이라 섞어 답하지 않는다. "번개"라는 말이 나오면 봉사 tool 이 아니라 번개 tool 이다.
   번개 tool 결과에는 **개최자·신청자 이름이 없다**(개인정보). 누가 여는지·누가 신청했는지 물으면 지어내지 말고 번개 게시판(\`flashLink\`)을 안내한다.
...
const HANDOFF_MARKS: readonly (string | RegExp)[] = [
  '제가 모르는',
  // 자료/정보/요약/기록 + (사이에 낀 목적어) + "없" · "들어 있지 않" · "찾지 못"
  //
  // ⚠ 사이를 비워 두면 안 된다(2026-08-28, 평가셋이 잡았다). 예전 정규식은 부정어가 **바로
  // 뒤에** 오는 형태만 봤는데, 모델은 목적어를 끼워 말한다:
  //   "제가 가진 정보에는 공지 본문에 특정 문구를 작성하는 방식에 대한 안내가 없습니다"
  // 이 답은 핸드오프인데 `handed_off = false` 로 남아, 회장단의 "답하지 못한 질문" 리포트에서
  // 빠졌다 — 결정 142 가 고쳤다고 적은 바로 그 집계 버그가 다른 모양으로 남아 있었다.
  //
  // 사이는 **한 문장 안, 40자까지**만 허용한다. 문장 부호를 넘겨 이으면 "정보에는 이렇게 있어요.
  // 다른 건 없고요" 같은 정상 답까지 핸드오프로 세게 된다.
  /(?:자료|정보|요약|기록)에(?:는)?[^.!?\n]{0,40}?(?:들어\s*있지\s*않|찾지\s*못|없)/,
];
