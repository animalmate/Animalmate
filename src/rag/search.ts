 ...
 * 모델에게 넘길 조각 수.
 *
 * 5 → 8 (2026-08-28). 청킹이 제목 없는 `##` 구분선을 인정하게 되면서 조각이 **잘아졌다**
 * (`33기 동아리 기본 정보` 2개 → 14개). 조각 하나가 좁고 선명해진 대신 자리를 다투는 조각이
 * 늘어, 5칸으로는 답이 있는 조각이 밀려난다 — 평가셋에서 "인스타그램 계정 알려주세요" 가
 * **7위**로 밀려 핸드오프됐다. 조각당 길이가 줄었으니 같은 프롬프트 예산으로 더 담을 수 있다.
 *
 * 무한정 넓히지 않는 이유: 무관한 자료를 잔뜩 넣으면 모델이 "자료에 없으면 핸드오프" 를 덜
 * 하게 되어 **지어낼 위험**이 커진다. 그 경계는 평가셋의 handoff·refuse 문항이 지킨다
 * (`npm run eval` — 8 로 올린 뒤 전부 통과하는 것을 확인했다).
 */
