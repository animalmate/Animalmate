...
export const RULES = {
   ...
   *
   * 값을 올려도 중복 지원은 늘지 않는다 — 실제 중복 방어는 IP 가 아니라 **이름+전화 덮어쓰기**다.
   * 같은 기수에 같은 사람이 몇 번을 내든 행은 하나로 유지되고 마지막 것만 남는다
   * (`submitApplicant`, 기수 행 FOR UPDATE 로 두 번 누르기 경합까지 막는다).
   * 모든 필드에 길이 상한이 걸려 있어 용량 공격도 막혀 있다.
