 ...
 *   면접이 끝난 사람에게 "일정을 확인하세요" 는 보내지 않는다.
...
export function resultMailContent(
  ...
  // "최종 **합격** 결과 안내" 라고 쓰지 않는다. 문법적으로는 중립이지만 제목만 훑으면 '합격'
  // 두 글자가 먼저 읽힌다 — 불합격자에게는 한 번 기대를 줬다 뺏는 제목이 된다.
  // (이 문구는 단위 테스트가 당락 단어를 금지해 잡아냈다.)
  const what =
    stage === 'document' ? '서류 심사 결과' : stage === 'interview' ? '면접 일정' : '최종 결과';

  const guide =
    stage === 'interview'
      ? '면접 일시와 장소(비대면이면 접속 링크)를 조회 화면에서 확인하실 수 있습니다.'
      : '조회 화면에서 지원하실 때 적으신 이름과 전화번호를 입력하시면 결과를 보실 수 있습니다.';

  return {
    subject: `[애니멀메이트] ${cohortLabel} ${what} 안내`,
    text:
      `안녕하세요, 애니멀메이트입니다.\n\n` +
      `${cohortLabel} ${what}가 나왔습니다. 아래 주소에서 확인해 주세요.\n\n` +
      `${lookupUrl}\n\n` +
      `${guide}\n\n` +
      // 메일 본문에 결과를 적지 않는 이유를 지원자에게도 알려 준다 — 안 그러면 "왜 결과가 안 적혀
      // 있지" 하고 스팸이나 오발송으로 의심한다.
      `개인정보 보호를 위해 결과는 메일에 적지 않고 조회 화면에서만 보여 드립니다.\n` +
