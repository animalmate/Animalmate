...
/**
 * 한 단계에 붙는 그림. PC 와 휴대폰 두 벌을 **다 만들되, 화면에는 탭으로 하나씩** 보여준다.
 *
 * 왜 둘 다 찍는가: 팀장단은 공지를 PC 로 걸어 두고 단톡 예약은 폰으로 하는 식으로 **기기를 섞어 쓴다.**
 * 한쪽만 보여주면 다른 기기를 쓰는 사람은 "내 화면은 왜 이렇게 안 생겼지"에서 멈춘다.
 * 특히 카카오톡은 PC 와 폰의 메뉴 생김새가 아예 다르다.
 *
 * 왜 나란히가 아니라 탭인가(2026-08-19): 나란히 놓으면 **모든 장에서 폭을 반씩 나눠 쓴다.**
 * 읽는 사람은 어차피 둘 중 한 기기 앞에 있는데, 1280px PC 캡처가 709px 로 줄어 캡처 속 글씨가
 * 7.2px 이 됐다(1440×820 실측). 탭으로 하나만 보여주면 같은 캡처가 그대로 커진다.
 *
...
export const WALK_STEPS: WalkStep[] = [
  ...
  },
  {
    key: 'template',
    title: '양식을 불러옵니다',
    body: '**양식 불러오기**에서 팀 양식을 고르면 제목·본문과 함께 **기본 장소·정원까지** 채워집니다. 팀장단은 본인 팀의 장소별 양식을 **템플릿**에서 바꿀 수 있습니다.',
    shots: { pc: 'template.pc.png', mobile: 'template.mobile.png' },
  },
  // 팀장단에게 물어보니 **여기가 실제로 제일 자주 틀리는 지점**이었다(2026-08-06 사용자 보고):
  // `{{ }}` 가 저절로 바뀌는 줄 모르고 지운 뒤 날짜를 직접 적는다.
  // 화면(PlaceholderGuide)은 이미 "무엇인지"를 설명하고 있으니, 여기서는 **손대지 않으면 무엇이
  // 좋아지는지**를 말한다 — 회차를 한 번에 여러 개 만들어도 날짜가 각각 맞게 들어간다는 것.
  // (겁주는 쪽보다 이득을 보여주는 쪽이 낫다는 사용자 판단, 2026-08-06.)
  {
    key: 'placeholders',
    title: '{{ }} 는 지우지 마세요',
    body: '중괄호 표시는 **손대지 않고 그대로 두면** 카페에 올라가는 순간 진짜 값으로 바뀝니다.\n\n회차를 여러 개 한 번에 만들면 **날짜도 각각 맞게 들어갑니다.**\n\n예약 후 **미리보기** 버튼을 통해 바뀐 글을 확인할 수 있어요.',
    shots: { pc: 'placeholders.pc.png', mobile: 'placeholders.mobile.png' },
  },
  {
    key: 'rows',
    title: '회차를 채웁니다',
    body: '세부 내용을 채워 주세요. 회차가 여러 번이면 **+ 일정 추가**로 줄을 늘리세요 — 회차마다 날짜·집합 시간·정원을 따로 적을 수 있습니다.',
    shots: { pc: 'rows.pc.png', mobile: 'rows.mobile.png' },
  },
  {
    key: 'preview',
    title: '올라갈 글을 눈으로 확인합니다',
    body: '**미리보기**는 `{{장소}}` 같은 자리를 실제 값으로 바꿔 보여줍니다. **카페에 올라간 글은 고칠 수 없으니** 여기서 한 번 읽고 저장하세요.',
  ...
  },
  {
    key: 'kakao-notice',
    title: '단톡방 공지문을 받습니다',
    body: '**카카오톡 공지 예약**을 누르면 보낼 문구와 **예약할 시간**(카페에 올라가는 그 시각)이 나옵니다. **공지문 복사**를 누르세요.',
