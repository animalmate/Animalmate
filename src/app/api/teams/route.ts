...
export async function GET(): Promise<Response> {
  ...
  // leaders 는 공지에 그대로 나가는 {{팀장단}} 문구(자동 명단+수동) — 작성 화면 미리보기가 발행 결과와 같도록 함께 준다.
  return NextResponse.json({
    teams: await Promise.all(scoped.map(async (t) => ({ id: t.id, name: t.name, leaders: await composeTeamLeaders(db, t.id) }))),
  });
}
